   (function() {
		...
		const AIRPLANE_SVG = `data:image/svg+xml;utf8,${encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M21 16v-2l-8-5V3.5c0-.83-.67-1.5-1.5-1.5S10 2.67 10 3.5V9l-8 5v2l8-2.5V19l-2 1.5V22l3.5-1 3.5 1v-1.5L13 19v-5.5l8 2.5z"/></svg>')}`;
		...
		function updateMarkersAndTracks() {
			...
			const start = idx >= 0 ? idx*26000 : 0;
			const end = start + 26000;
		...
		function updateInfoTables(allFlights) {
			...
			unidentified.sort((a,b) => a.hex.localeCompare(b.hex)).slice(0, 200).forEach(f => {
				...
				tr.innerHTML = `<td>${f.hex}</td><td>${f.lat.toFixed(2)}</td><td>${f.lon.toFixed(3)}</td><td>${agl}</td><td>${f.spd_kmh ?? '—'}</td>`;
		...
		async function resolveCountry(lat, lon) {
			const key = `${lat.toFixed(2)},${lon.toFixed(3)}`;
		...
		function viewGroupDetails(country, group) {
			...
			const groupFlights = Array.from(flightsByHex.values()).filter(f => {
				const fCountry = countryCache.get(`${f.lat.toFixed(2)},${f.lon.toFixed(3)}`) || 'Unknown';
		...
		function getAircraftGalleryUrl(f) {
			...
			return `https://www.airplane-pictures.net/search.php?search=${encodeURIComponent(f.acType)}`;
		}
		...
		function startAutoUpdate() { if (autoUpdateTimer) clearInterval(autoUpdateTimer); autoUpdateTimer = setInterval(refresh, 3000); }
